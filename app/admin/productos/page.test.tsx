...
const { dbMock } = vi.hoisted(() => ({
  dbMock: {
    product: {
      findMany: vi.fn(async ({ where }: { where?: { categoryId?: string; isPublished?: boolean } } = {}) => {
        const products = [
          {
            id: "prod-1",
            sku: "SKU-1",
      ...
      findMany: vi.fn(async () => [
        {
          ...
          isActive: true,
          products: [
            { id: "prod-1", isPublished: true },
            { id: "prod-2", isPublished: false },
          ],
        },
        {
          id: "cat-2",
          ...
          isActive: true,
          products: [{ id: "prod-3", isPublished: true }],
        },
      ]),
    },
  },
...
describe("AdminProductsPage", () => {
  it("shows category publication summary and batch actions", async () => {
    ...
    expect(markup).toContain("Selección manual sobre la vista actual");
    ...
    expect(markup).toContain("Mostrando 3 totales del catálogo completo");
  ...
  it("shows category drill-down context when a category is selected", async () => {
    ...
    expect(markup).toContain("Desde acá decidís producto por producto sin perder la acción masiva de Almacén");
    expect(markup).toContain("Categoría activa");
    expect(markup).toContain("Publicados en vista");
    expect(markup).toContain("Sin publicar en vista");
    expect(markup).toContain("Viendo ítems");
    expect(markup).toContain("Mostrando 2 totales de la categoría seleccionada");
    expect(markup).toContain("/admin/productos?categoryId=cat-1#items");
  });
});
